...
app.post('/api/parse-cv', async (req, res) => {
  try {
    ...
    logger.info('CV parsing request received', { fileType });
    
    const result = await cvParsingService.parseCV(fileData, fileType);
    
    logger.info('CV parsing completed successfully');
    res.json(result);
    
  } catch (error) {
    logger.error('CV parsing endpoint error', { error: error.message });
    res.status(500).json({ 
  ...
  try {
    if (!vectorService) {
      return res.status(503).json({ 
        error: 'Vector service not available - database connection required' 
      });
    }
