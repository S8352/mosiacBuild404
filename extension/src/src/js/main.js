...
class JobCopilotApp {
  ...
  initEventListeners() {
    ...
    document.querySelectorAll('.remove-experience-btn').forEach(btn => {
      btn.addEventListener('click', (e) => this.removeExperience(e.target));
    });
    ...
    if (editProfileBtn) {
      editProfileBtn.addEventListener('click', () => this.previousStep());
    }

    // Voice panel
    const closeVoicePanelBtn = document.getElementById('close-voice-panel-btn');
    if (closeVoicePanelBtn) {
      closeVoicePanelBtn.addEventListener('click', () => this.closeVoicePanel());
    }
  ...
  showHelp() {
    ...
    alert('Job Copilot Help:\n\n1. Fill in your profile information\n2. Upload your CV or use voice input\n3. Navigate through the steps\n4. Complete your profile to start using Job Copilot');
  }
